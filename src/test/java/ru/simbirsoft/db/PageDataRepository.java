...
public class PageDataRepository {
    ...
    public int createPublishedPageEntity(String title, String content) {
        String sql = """
        ...
        try (Connection connection = DatabaseClient.getConnection();
            ...
            throw new IllegalStateException("Не удалось создать страницу в таблице wp_posts", sqlException);
        }
    }

    public void deletePageId(int pageId) {
        String sql = "DELETE FROM wp_posts WHERE ID = ? AND post_type = 'page'";
        ...
        try (Connection connection = DatabaseClient.getConnection();
            ...
            throw new IllegalStateException("Не удалось удалить страницу из таблицы wp_posts", exception);
        }
    }
}
